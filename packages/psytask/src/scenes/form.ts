...
const init = <T extends Primitive>(
  ...
  return { element: root, data: () => ({ value: getValue() }) };
};
...
export const Form = function <T extends Record<string, FieldRecord>>(
  ...
  const titleEl = h('h2', { style: { marginBottom: '0.5rem' } });
  ...
  const submitButton = h('button', {
    ...
    onclick() {
      formEl.checkValidity() ? self.close() : formEl.reportValidity();
    },
  });
  ...
  effect(() => {
    if (props.fields) {
      ...
      fieldContainer.replaceChildren(...fields.map((f) => f.element));
    }
  });
