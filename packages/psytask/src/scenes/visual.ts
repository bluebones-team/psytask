...
import { effect, reactive, type Reactive } from '../reactive';
...
export const TextStim = function (
  props: Reactive<
    CSSProperties & { children?: string | Node | (string | Node)[] }
  >,
) {
  const el = h('div', {
...
export const ImageStim = function (
  props: Reactive<
...
export const VirtualChinrest = function (
  ...
  const state = reactive({
    // screen width
    line_spacing_pix: Math.floor(self.app.data.window_wh_pix[0] / 2),
    line_spacing_cm: 0,
    pix_per_cm: 0,
    screen_width_cm: 0,
    // visual distance
  ...
  effect(() => {
    state.pix_per_cm = self.app.data.screen_wh_pix[0] / state.screen_width_cm;
  });
  effect(() => {
    state.screen_width_cm = self.app.data.screen_wh_pix[0] / state.pix_per_cm;
  });
  effect(() => {
    ...
    content: (() => {
      ...
      effect(() => {
        movableLine.style.left =
          state.line_spacing_pix / self.app.data.dpr + 'px';
      });
      self
        ...
        .on('pointermove', (e) => {
          ...
          state.line_spacing_pix = (e.clientX - sx) * self.app.data.dpr;
        });
    ...
    content: (() => {
      ...
      effect(() => {
        movableObj.style.right =
          size + state.move_width_pix * self.app.data.dpr + 'px';
      });
      self.on('scene:frame', () => {
          ...
          ...state.move_widths.map(({ pix, cm }) =>
            h('span', {
              ...
              style: {
                ...
                right: size + pix * self.app.data.dpr + 'px',
  ...
  effect(() => {
    ...
    if (!sValue || usePreviousData === false) {
      textProps.children = screenWidthEL;
      return;
    }
      ...
      h(
        ...
        [
          ...
          },
          {
            ...
            onclick: () => (textProps.children = screenWidthEL),
          },
        ].map((p) => h('button', p)),
      ),
    ];
